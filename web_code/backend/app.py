...
def ingest():
    ...
    if rtype == "GPS":
        ...
        sse_event("gps", reading_to_dict(r))     # ← GPS도 항상 전송
...
def stats_exceed():
    rk = request.args.get("range","24h")
    return jsonify(_exceed_counts(rk))
...
def admin_clear_recent():
    ...
    if ids:
        db.session.execute(text("DELETE FROM evidence     WHERE event_id IN :ids"), {"ids": tuple(ids)})
        db.session.execute(text("DELETE FROM action_logs  WHERE event_id IN :ids"), {"ids": tuple(ids)})
        db.session.execute(text("DELETE FROM sop_logs     WHERE event_id IN :ids"), {"ids": tuple(ids)})
        db.session.execute(text("DELETE FROM cosigns      WHERE event_id IN :ids"), {"ids": tuple(ids)})
        db.session.execute(text("DELETE FROM events       WHERE id       IN :ids"), {"ids": tuple(ids)})

    # readings 도 해당 시점 이후 삭제
